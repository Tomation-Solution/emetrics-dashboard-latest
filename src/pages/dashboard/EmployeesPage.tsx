...
import { employeesPageDashboardData } from "../../data/dashboard/employeesPageDashboardData";
...
const EmployeesPage = () => {
  return (
    ...
    <div className="grid grid-cols-1 gap-5 sm:grid-cols-3 md:grid-cols-3">
      {employeesPageDashboardData.map((data, index) => (
        <DashboardCard
          key={index}
          title={data.title}
          value={data.value}
    ...
    <div className="w-full">
      <TableMetaData title={"Employees"}  hideFilter={true} />
      <EmployeesTable />
    </div>
